...
func (i *Injector) typeCheck(fset *token.FileSet, files []*ast.File) (map[*ast.Ident]types.Object, map[ast.Expr]types.TypeAndValue, error) {
	...
	if err := checker.Files(files); err != nil {
		return nil, nil, fmt.Errorf("type-checking files: %w", err)
	}

	return typeInfo.Uses, typeInfo.Types, nil
}
