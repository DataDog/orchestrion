	...
	*/
	...
	newLines := []dst.Stmt{
		&dst.AssignStmt{
			...
			Rhs: []dst.Expr{
				&dst.CallExpr{
					Fun:  &dst.Ident{Name: "Report", Path: "github.com/datadog/orchestrion/orchestrion"},
...
func buildArgs(contextExpr contextInfo, event event.Event, name string, parts []string) []dst.Expr {
	...
	out = append(out,
		...
		&dst.Ident{Name: event.String(), Path: "github.com/datadog/orchestrion/orchestrion/event"},
			...
			if iden, ok := fun.Fun.(*dst.Ident); ok {
				...
				for _, opt := range opts {
					fun.Args = append(fun.Args,
						&dst.CallExpr{Fun: &dst.Ident{Name: opt, Path: "github.com/datadog/orchestrion/orchestrion"}},
					)
				}
			}
		}
			...
			for _, e := range x.Elts {
				...
				if kve, ok := e.(*dst.KeyValueExpr); ok {
					...
					kve.Value = &dst.CallExpr{
						Fun:  &dst.Ident{Name: "WrapHandler", Path: "github.com/datadog/orchestrion/orchestrion"},
...
func wrapClientFromAssign(stmt *dst.AssignStmt, tc *typechecker.TypeChecker) bool {
	...
	*/
	...
	stmt.Rhs[0] = &dst.CallExpr{
		Fun:  &dst.Ident{Name: "WrapHTTPClient", Path: "github.com/datadog/orchestrion/orchestrion"},
...
func wrapSqlCall(call *dst.CallExpr) bool {
	...
	f.Path = "github.com/datadog/orchestrion/orchestrion/sql"
	return true
}
	...
	*/
	wrap := func(fun *dst.CallExpr, name string) bool {
		...
		fun.Args[1] = &dst.CallExpr{
			Fun:  &dst.Ident{Name: wrapper, Path: "github.com/datadog/orchestrion/orchestrion"},
		...
		Body: &dst.BlockStmt{
			List: []dst.Stmt{
							...
							Args: []dst.Expr{
								&dst.CallExpr{
									...
									Args: []dst.Expr{
										...
										&dst.Ident{Name: "EventCall", Path: "github.com/datadog/orchestrion/orchestrion/event"},
				...
				&dst.AssignStmt{
					...
					Rhs: []dst.Expr{
						&dst.CallExpr{
							Fun:  &dst.Ident{Name: "InsertHeader", Path: "github.com/datadog/orchestrion/orchestrion"},
				...
				&dst.DeferStmt{Call: &dst.CallExpr{
					...
					Args: []dst.Expr{
						...
						&dst.Ident{Name: "EventReturn", Path: "github.com/datadog/orchestrion/orchestrion/event"},
						&dst.BasicLit{Kind: token.STRING, Value: `"name"`},
						&dst.SelectorExpr{
							X:   &dst.Ident{Name: requestName},
							Sel: &dst.Ident{Name: "URL"},
...
func addInit(decl *dst.FuncDecl) *dst.FuncDecl {
	...
	newLines := []dst.Stmt{
		&dst.DeferStmt{
			Call: &dst.CallExpr{
				Fun: &dst.CallExpr{
					Fun: &dst.Ident{Path: "github.com/datadog/orchestrion/orchestrion", Name: "Init"},
				},
			},
			Decs: dst.DeferStmtDecorations{NodeDecs: dst.NodeDecs{
					...
					Args: []dst.Expr{
						&dst.CallExpr{
							...
							Args: []dst.Expr{
								...
								&dst.Ident{Name: "EventStart", Path: "github.com/datadog/orchestrion/orchestrion/event"},
								&dst.BasicLit{Kind: token.STRING, Value: `"name"`},
								dup(funcName),
								&dst.BasicLit{Kind: token.STRING, Value: `"verb"`},
								&dst.SelectorExpr{
		...
		&dst.DeferStmt{
			Call: &dst.CallExpr{
				...
				Args: []dst.Expr{
					...
					&dst.Ident{Name: "EventEnd", Path: "github.com/datadog/orchestrion/orchestrion/event"},
					&dst.BasicLit{Kind: token.STRING, Value: `"name"`},
					dup(funcName),
					&dst.BasicLit{Kind: token.STRING, Value: `"verb"`},
					&dst.SelectorExpr{
